...
def getListFiles(path):
    ...
    iles 是 list , 内容是该文件夹中所有的文件(不包括子目录)
    函数功能是将所有文件的路径加入ret数组，返回
    '''
...
def get_images_labels():
    #获取符号的图片，以及图片对应的标签（该图片是什么符号）
    operators = ['plus', 'sub', 'mul', 'div', '(', ')']
    images = None
    labels = None
    for i, op in enumerate(operators):
        #image_file_list每个文件的路径
    ...
    '''
    返回值 ：images = [images_size , 28*28]
            labels = [images_size,16]
    '''
